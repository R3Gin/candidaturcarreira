...
const title = "Minha conta de freelancer | Candidatu";
const description =
  "Veja seus jobs já feitos, nível da conta, ranking e avaliação das empresas, além do total de diárias e do dinheiro ganho nos freelas da Candidatu.";

export const Route = createFileRoute("/freelance/conta")({
  head: () => ({
    meta: [
      { title },
...
function ContaFreelaPage() {
  ...
  const m = useMemo(() => computeMetrics(acc), [acc]);
    ...
    <main>
      ...
      <section className="brand-gradient text-primary-foreground">
        <div className="mx-auto max-w-6xl px-5 py-12 md:py-16">
          <p className="eyebrow text-primary-foreground/60">Conta de freelancer</p>
          <div className="mt-3 flex flex-col gap-5 sm:flex-row sm:items-center">
            <span className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-primary-foreground/10 text-xl font-bold">
              {acc.avatar ? (
                <img src={acc.avatar} alt={acc.nome} className="h-full w-full object-cover" />
              ) : (
                acc.nome.slice(0, 2).toUpperCase()
              )}
            </span>
            <div className="min-w-0">
              <h1 className="font-display text-2xl font-bold md:text-3xl">{acc.nome}</h1>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1.5 rounded-full bg-primary-foreground/10 px-3 py-1 text-xs font-bold">
                  ...
                  {m.rating.toFixed(2)} ({m.avaliacoes} avaliações)
                </span>
                <span className="inline-flex items-center gap-1.5 rounded-full bg-primary-foreground/10 px-3 py-1 text-xs font-bold">
                  <Trophy className="h-3.5 w-3.5" strokeWidth={2} />
                  Top {Math.max(1, Math.round((m.posicao / m.totalFreelas) * 100))}% · #{m.posicao}
                </span>
              </div>
            </div>
            <Link
              to="/freelance"
              className="sm:ml-auto rounded-full bg-primary-foreground px-5 py-2.5 text-center text-sm font-semibold text-primary"
            >
              Buscar freelas
            </Link>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-5 py-10 md:py-14">
        <ul className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {stats.map((s, i) => (
            <Reveal as="li" key={s.label} delay={i * 60} className="surface-card rounded-2xl p-4">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <aside className="space-y-6">
            <section className="surface-card rounded-2xl p-5">
              ...
              <h2 className="mt-1 font-display text-lg font-semibold text-ink">Como as empresas te avaliam</h2>
              ...
              <p className="mt-4 rounded-xl bg-secondary p-3 text-xs leading-relaxed text-ink-soft">
                Média das últimas avaliações, como no app de corridas: notas de 1 a 5 dadas pela
                empresa no fim de cada diária. Abaixo de 4,0 sua conta entra em revisão.
              </p>
            </section>
              ...
              <div className="mt-4 border-t border-border pt-4">
                ...
                {m.nextLevel && (
                  <p className="mt-2 text-xs text-ink-soft">
                    Faltam {Math.max(0, m.nextLevel.min - m.diarias)} diárias para o nível{" "}
                    {m.nextLevel.nome}.
                  </p>
                )}
              </div>
    ...
    <form onSubmit={submit} className="surface-card rounded-2xl p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <label className="block sm:col-span-2">
          ...
          <input value={cargo} onChange={(e) => setCargo(e.target.value)} className={field} placeholder="Barman para casamento" />
        </label>
        <label className="block">
          <span className={label}>Empresa</span>
          <input value={empresa} onChange={(e) => setEmpresa(e.target.value)} className={field} placeholder="Vértice Eventos" />
        </label>
        <label className="block">
          <span className={label}>Diárias</span>
          <input type="number" min={1} value={diarias} onChange={(e) => setDiarias(Math.max(1, Number(e.target.value)))} className={field} />
        </label>
        <label className="block">
          <span className={label}>Valor recebido (R$)</span>
          <input type="number" min={0} step={10} value={valor} onChange={(e) => setValor(Number(e.target.value))} className={field} />
        </label>
        <label className="block">
